<template>
    <div>
        ...
        <form action="#" @submit.prevent=" edit ? updateContact(contact.id): createContact()">
            ...
            <div class="form-group">
                <button v-show="!edit" class="btn btn-primary" type="submit">New Contact</button>
                <button v-show="edit" class="btn btn-primary" type="submit">Update Contact</button>
            </div>
        </form>
        <h1>Contact List</h1>
        <ul class="list-group">
            <li v-bind:key="contact.id" class="list-group-item" v-for="contact in list">
                <strong>{{contact.name}}</strong> {{contact.email}} {{contact.phone}}
                <button @click="showContact(contact.id)" class="btn btn-info btn-xs">Edit</button>
                <button @click="deleteContact(contact.id)" class="btn btn-danger btn-xs">Del</button>
            </li>
        </ul>
    </div>
</template>
